...
class TestFactionLoader(unittest.TestCase):
    ...
    def test_load_valid_faction(self) -> None:
        with (RESOURCES_DIR / "valid_faction.json").open("r") as data:
            ...
            self.assertIn(F_15C, faction.all_aircraft)
            self.assertIn(F_15E, faction.all_aircraft)
            self.assertIn(F_14B, faction.all_aircraft)
            self.assertIn(FA_18C_hornet, faction.all_aircraft)
            self.assertIn(F_16C_50, faction.all_aircraft)
            self.assertIn(A_10A, faction.all_aircraft)
            self.assertIn(AV8BNA, faction.all_aircraft)
            self.assertIn(UH_1H, faction.all_aircraft)
            self.assertIn(AH_64A, faction.all_aircraft)
            self.assertIn(B_52H, faction.all_aircraft)
            self.assertIn(B_1B, faction.all_aircraft)
            self.assertIn(F_117A, faction.all_aircraft)
            self.assertIn(A_10C, faction.all_aircraft)
            self.assertIn(A_10C_2, faction.all_aircraft)
